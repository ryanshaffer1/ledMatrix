...
void plasma_gradient() {
  ...
  long        hueShift= 0;  
  #define FPS 5         // Maximum frames-per-second
  ...
  while(1) {
    ...
    // ---------------- Do math to determine the hue value for each pixel ----------------  
    ...
    // ---------------- DISPLAY ----------------
    // Iterate through each pixel (y and x values)
    for(y=0; y<mh; y++) {
      x1 = sx1; x2 = sx2; x3 = sx3; x4 = sx4;
      for(x=0; x<mw; x++) {
        // Calculate hue of the current pixel based on the value computed earlier
        value = hueShift
        ...
        CHSV hsv(value * 3, 255, 255);
        // Draw that hue to the screen at the specific pixel
        matrix->drawPixel(x, y, hsv);
        x1--; x2--; x3--; x4--;
      }
      y1--; y2--; y3--; y4--;
    }

    matrix->show(); // Show all updates
